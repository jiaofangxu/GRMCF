...
# def stay_time(dx, dy, dz):
#     """
#     改进版聚焦等待时间模型（基于真实实验数据）：
#     1) 绝大多数聚焦时间落在 0.08 ~ 0.10s
#     2) 少数极端情况可达到 0.18s
#     3) 使用高斯分布 + 尾部补偿来建模真实行为
#     """
#
#     if dx == 0 and dy == 0 and dz == 0:
#         return 0.0
...
#     # 基础聚焦时间（真实数据多数集中于此）
#     base_mean = 0.09     # 中心值
#     base_std  = 0.02     # 波动范围（越小越稳定）
#
#     # 高斯随机波动（模拟对焦复杂度）
#     noise = random.gauss(0, base_std)
#
#     # 针对极端变焦情况，轻微增加等待时间（仅当 dz 特别大时起作用）
#     max_extra = 0.06     # 允许最多 0.06s 额外等待 → 可接近 0.15s
#     dz_effect = max_extra * (1 - math.exp(-abs(dz) / 12))  # 异常情况才起效
#
...
def stay_time(dx, dy, dz):
    """
    改进后的聚焦时间模型：
    针对 dx、dy、dz 位移很小的情况，加入‘精调补偿项（fine-tune penalty）’
    提升对真实环境中对焦 hunting 行为的拟合能力。
    """
    ...
    if max_axis < 15:  # 实验可调阈值
        fine_tune = 0.12 * math.exp(-max_axis / 8)  # 越小补偿越大
    else:
        fine_tune = 0.0
